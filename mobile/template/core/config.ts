...
import { LiveHubConfig } from './types';

class ConfigManager {
  private static instance: ConfigManager;
  private config: LiveHubConfig | null = null;
  ...
  public initialize(config: LiveHubConfig) {
    if (this.config) {
  ...
  public getConfig(): LiveHubConfig {
    if (!this.config) {
  ...
  private createDefaultConfig(): LiveHubConfig {
  ...
  public updateConfig(updates: Partial<LiveHubConfig>) {
    const config = this.getConfig();
...
export const createConfig = (options: CreateConfigOptions = {}): LiveHubConfig => {
  let authConfig;
